                ...
                <form action="{{ route('LevelKaryawan.update', $levelKaryawan->id) }}" method="POST">
                    ...
                    <div class="mt-6">
                        ...
                        <textarea name="deskripsi" id="deskripsi" rows="3"
                            class="w-full border border-gray-300 rounded-lg px-4 py-2 bg-gray-50 focus:outline-none focus:ring-2 focus:ring-blue-500">{{ old('deskripsi', $levelKaryawan->deskripsi) }}</textarea>
                        @error('deskripsi')
                            <span class="text-sm text-red-600">{{ $message }}</span>
                        @enderror
                    </div>
