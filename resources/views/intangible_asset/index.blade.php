            ...
            <div class="bg-white rounded-2xl shadow-lg overflow-hidden border border-gray-200">
                ...
                <div class="sticky top-0 z-20 px-6 py-5 border-b border-gray-100 flex justify-between items-center"
                    ...
                    <div class="flex flex-wrap gap-2">
                        ...
                        @can('intangible_asset.create')
                            <a href="{{ route('intangible_asset.create') }}"
                                ...
                                <i class="fas fa-plus mr-2"></i> Add Intangible Asset
                            </a>
                        @endcan
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50 sticky top-0 z-10">
                            <tr>
                                ...
                                <th
                                    class="px-4 py-2 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">
                                    Masa Manfaat(Dalam Tahun)</th>
                                <th
                                    class="px-4 py-2 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">
                                    Metode Amortisasi</th>
                                <th
                                    class="px-4 py-2 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">
                                    Tarif Amortisasi</th>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @forelse ($data as $item)
                                <tr class="hover:bg-gray-50 transition-colors duration-150">
                                    ...
                                    <td class="px-2 py-1 text-center">{{ $item->kode_asset }}</td>
                                    <td class="px-2 py-1 text-center">{{ $item->nama_asset }}</td>
                                    <td class="px-2 py-1 text-center">{{ $item->kategori->nama_kategori ?? '-' }}</td>
                                    <td class="px-2 py-1 text-center">{{ $item->deskripsi }}</td>
                                    <td class="px-2 py-1 text-center">{{ $item->brand ?? '-' }}</td>
                                    <td class="px-2 py-1 text-center">{{ $item->lokasi->nama_lokasi ?? '-' }}</td>
                                    <td class="px-2 py-1 text-center">{{ $item->golongan->nama_golongan ?? '-' }}</td>
                                    <td class="px-2 py-1 text-center">{{ $item->dalam_tahun }}</td>
                                    <td class="px-2 py-1 text-center">{{ $item->metode_penyusutan->nama_metode ?? '-' }}
                                    </td>
                                    <td class="px-2 py-1 text-center">{{ $item->tarif_amortisasi }}</td>

                                    <td class="px-2 py-1 text-right">
                                        <div class="flex justify-end space-x-3">
                                            @can('intangible_asset.view')
                                                <a href="{{ route('intangible_asset.show', $item->id) }}"
                                            ...
                                            @can('intangible_asset.update')
                                                <a href="{{ route('intangible_asset.edit', $item->id) }}"
                                            ...
                                            @can('intangible_asset.delete')
                                                <form id="delete-form-{{ $item->id }}"
                                                    action="{{ route('intangible_asset.destroy', $item->id) }}" method="POST"
                                                    style="display: none;">
                                                    @csrf
                                                    @method('DELETE')
                                                </form>
                                ...
                                <tr>
                                    <td colspan="15" class="px-6 py-12 text-center">
                                        <div class="flex flex-col items-center text-gray-400">
                                            ...
                                            <p class="text-lg font-medium">Belum ada Intangible Asset</p>
                                            <a href="{{ route('tangible_asset.create') }}"
    ...
    <div id="fileModal"
        ...
        <div class="bg-white rounded-xl shadow-lg p-6 w-full max-w-md">
            ...
            <div class="space-y-3 text-sm text-gray-700">
                <a href="{{ asset('template/template_import_intangible.xlsx') }}" download
                    class="block hover:bg-gray-50 p-2 rounded-lg">
                    <i class="fas fa-file-excel mr-2 text-green-600"></i> Download Template Excel
                </a>
                <a href="{{ route('export.IntangibleAsset') }}" class="block hover:bg-gray-50 p-2 rounded-lg">
                    <i class="fas fa-file-download mr-2 text-blue-500"></i> Export Project
                </a>
                <form action="{{ route('import.IntangibleAsset') }}" method="POST" enctype="multipart/form-data"
                    class="space-y-2">
                    @csrf
