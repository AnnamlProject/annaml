                    ...
                    <div class="flex flex-wrap gap-2">
                        ...
                        @can('ptkp.create')
                            <a href="{{ route('ptkp.create') }}"
                                ...
                                <i class="fas fa-plus mr-2"></i> Add PTKP
                            </a>
                        @endcan
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @forelse ($ptkp as $item)
                            <tr class="hover:bg-gray-50 transition-colors duration-150">
                                ...
                                <td class="px-2 py-1 text-right">{{ number_format($item->nilai) }}</td>
                                <td class="px-2 py-1 text-right">
                                    <div class="flex justify-end space-x-3">
                                        @can('ptkp.view')
                                            <a href="{{ route('ptkp.show', $item->id) }}"
                                        ...
                                        @can('ptkp.delete')
                                            <form id="delete-form-{{ $item->id }}"
                                                action="{{ route('ptkp.destroy', $item->id) }}" method="POST"
                                                style="display: none;">
                                                @csrf
                                                @method('DELETE')
                                            </form>
                            ...
                            <tr>
                                <td colspan="7" class="px-6 py-12 text-center">
                                    <div class="flex flex-col items-center text-gray-400">
                                        ...
                                        <p class="text-lg font-medium">Belum ada PTKP</p>
                                        <a href="{{ route('ptkp.create') }}"
    ...
    <div id="fileModal"
        ...
        <div class="bg-white rounded-xl shadow-lg p-6 w-full max-w-md">
            ...
            <div class="space-y-3 text-sm text-gray-700">
                <a href="{{ asset('template/template_import_ptkp.xlsx') }}" download
                    class="block hover:bg-gray-50 p-2 rounded-lg">
                    <i class="fas fa-file-excel mr-2 text-green-600"></i> Download Template Excel
                </a>
                <a href="{{ route('export.Ptkp') }}" class="block hover:bg-gray-50 p-2 rounded-lg">
                    <i class="fas fa-file-download mr-2 text-blue-500"></i> Export Project
                </a>
                <form action="{{ route('import.Ptkp') }}" method="POST" enctype="multipart/form-data" class="space-y-2">
                    @csrf
