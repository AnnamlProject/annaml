            ...
            <div class="bg-white rounded-2xl shadow-lg overflow-hidden border border-gray-200">
                ...
                <div class="sticky top-0 z-20 px-6 py-5 border-b border-gray-100 flex justify-between items-center"
                    ...
                    <div class="flex flex-wrap gap-2">
                        ...
                        @can('Transfer inventory.create')
                            <a href="{{ route('transfer_inventory.create') }}"
                                ...
                                <i class="fas fa-plus mr-2"></i> Add Transfer Inventory
                            </a>
                        @endcan
                    </div>
                </div>
                        ...
                        @forelse($data as $item)
                            <tr class="hover:bg-gray-50 transition-colors duration-150">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm font-medium text-gray-900">{{ $item->date }}
                                    </div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm text-gray-900">{{ $item->fromInventory->kode_lokasi }}</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm text-gray-900">{{ $item->toInventory->kode_lokasi }}</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <div class="flex justify-end space-x-3">
                                        @can('Transfer inventory.view')
                                            <a href="{{ route('transfer_inventory.show', $item->id) }}"
                                        ...
                                        @can('Transfer inventory.update')
                                            <a href="{{ route('transfer_inventory.edit', $item->id) }}"
                                        ...
                                        @can('Transfer inventory.delete')
                                            <form id="delete-form-{{ $item->id }}"
                                                action="{{ route('transfer_inventory.destroy', $item->id) }}" method="POST"
                                                style="display: none;">
                                                @csrf
                                                @method('DELETE')
                                            </form>
                            ...
                            <tr>
                                <td colspan="9" class="px-6 py-12 text-center">
                                    <div class="flex flex-col items-center justify-center text-gray-400">
                                        ...
                                        <p class="text-lg font-medium text-gray-500">No Transfer Inventory found</p>
                                        <a href="{{ route('transfer_inventory.create') }}"
                                            class="mt-4 inline-flex items-center px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition">
                                            <i class="fas fa-plus mr-2"></i> Create First Transfer Inventory
                                        </a>
                                    </div>
                                </td>
                            </tr>
