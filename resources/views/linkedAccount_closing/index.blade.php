            ...
            <div class="bg-white rounded-2xl shadow-lg overflow-hidden border border-gray-200">
                ...
                <div class="sticky top-0 z-20 px-6 py-5 border-b border-gray-100 flex justify-between items-center"
                    ...
                    <h3 class="text-xl font-bold text-white flex items-center">
                        ...
                        Linked Account Closing
                    </h3>
                    <a href="{{ route('linkedAccount_closing.create') }}"
                        ...
                        <i class="fas fa-plus mr-2"></i> Add Linked Account
                    </a>
                </div>
            </div>
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @forelse ($data as $item)
                            <tr class="hover:bg-gray-50 transition-colors duration-150">
                                ...
                                <td class="px-6 py-4">{{ $item->unitKerja->nama_unit }}</td>
                                <td class="px-6 py-4">{{ $item->kode }}</td>
                                @if (is_null($item->departemen_id))
                                    <td class="px-6 py-4">
                                        {{ optional($item->akun)->kode_akun . ' - ' . optional($item->akun)->nama_akun ?? '-' }}
                                    </td>
                                @else
                                    <td class="px-6 py-4">
                                        {{ optional($item->akun)->kode_akun . ' - ' . optional($item->akun)->nama_akun . ' - ' . optional($item->departemen)->deskripsi ?? '-' }}
                                    </td>
                                @endif
                                ...
                                <td class="px-6 py-4 text-right">
                                    <div class="flex justify-end space-x-3">
                                        <form id="delete-form-{{ $item->id }}"
                                            action="{{ route('linkedAccount_closing.destroy', $item->id) }}" method="POST"
                                            style="display: none;">
                                            @csrf
                                            @method('DELETE')
                                        </form>
                            ...
                            <tr>
                                <td colspan="7" class="px-6 py-12 text-center">
                                    <div class="flex flex-col items-center text-gray-400">
                                        ...
                                        <p class="text-lg font-medium">Belum ada Linked akun Sales</p>
                                        <a href="{{ route('linkedAccount_closing.create') }}"
