...
@section('content')
                    ...
                    <div class="grid grid-cols-3 gap-4 text-sm">
                        <div>
                            ...
                            <input type="text" class="w-full border bg-gray-100 rounded px-2 py-1 text-sm"
                                value="{{ old('no_pengajuan', $pengajuan->no_pengajuan ?? '-') }}"
                    ...
                    <div class="overflow-x-auto border rounded-lg shadow-sm mt-6">
                        <table class="w-full border-collapse border text-sm whitespace-nowrap">
                            ...
                            <tbody id="tbody-approval">
                                    ...
                                    <tr>
                                        ...
                                        <td style="padding: 12px; border: 1px solid #ddd;">
                                            <select name="account_id[]"
                                                ...
                                                @foreach ($account as $g)
                                                    <option value="{{ $g->id }}"
                                                        {{ $detail->account_id == $g->id ? 'selected' : '' }}>
                                        ...
                                        <td style="padding: 12px; border: 1px solid hsl(0, 0%, 87%);">
                                            <input type="text" name="harga[]"
                                                ...
                                                class="w-full number-format border rounded-lg px-4 py-2 bg-gray-50">
                                        </td>
                                        <td style="padding: 12px; border: 1px solid hsl(0, 0%, 87%);">
                                            <input type="text" name="uraian[]" value="{{ $detail->uraian }}"
                                                class="w-full border rounded-lg px-4 py-2 bg-gray-50">
                                        </td>
                                ...
                                @empty
                                    {{-- Kalau tidak ada detail, tampilkan 1 baris kosong seperti create --}}
                                    <tr>
                                        ...
                                        <td style="padding: 12px; border: 1px solid #ddd;">
                                            <select name="account_id[]"
                                                ...
                                                <option value="">-- Pilih Account --</option>
                                                @foreach ($account as $g)
                                                    <option value="{{ $g->id }}">{{ $g->kode_akun }} -
                                                        {{ $g->nama_akun }}</option>
                                                @endforeach
                                            </select>
                                        </td>
                                        <td><input type="text" name="qty[]"
                                                class="w-full border rounded-lg px-4 py-2 bg-gray-50"></td>
                                        <td><input type="text" name="harga[]"
                                                class="w-full number-format border rounded-lg px-4 py-2 bg-gray-50"></td>
                                        <td><input type="text" name="uraian[]"
                                                class="w-full border rounded-lg px-4 py-2 bg-gray-50"></td>
                                        <td style="text-align:center;"><button type="button" onclick="hapusBaris(this)"
                                                style="color:red;border:none;background:none;">🗑️</button></td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
    ...
    <script>
        ...
        <td style="padding: 12px; border: 1px solid #ddd;">
            <select name="account_id[]" 
                ...
                <option value="">-- Pilih Account --</option>
                @foreach ($account as $g)
                    <option value="{{ $g->id }}">{{ $g->kode_akun }} - {{ $g->nama_akun }}</option>
                @endforeach
            </select>
        </td>
        <td style="padding: 12px; border: 1px solid hsl(0, 0%, 87%);">
    ...
    `;
            ...
            // ✅ Reinit Select2 untuk select baru
            ...
            // ✅ Tambahkan event number format untuk input baru
            $(row).find('.number-format').on('input', function() {
                let value = this.value.replace(/\D/g, '');
                this.value = new Intl.NumberFormat('id-ID').format(value);
            });
        }
