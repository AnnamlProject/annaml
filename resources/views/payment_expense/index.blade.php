        ...
        <div class="max-w-full mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="bg-white rounded-2xl shadow-lg overflow-hidden border border-gray-200">
                ...
                <div class="sticky top-0 z-20 px-6 py-5 border-b border-gray-100 flex justify-between items-center"
                    ...
                    @can('payment_expense.create')
                        <a href="{{ route('payment_expense.create') }}"
                            ...
                            <i class="fas fa-plus mr-2"></i> Add Payment Expense
                        </a>
                    @endcan
                </div>
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @forelse ($data as $item)
                            <tr class="hover:bg-gray-50 transition-colors duration-150">
                                ...
                                <td class="px-6 py-2">{{ $item->Account->nama_akun ?? '-' }}</td>
                                <td class="px-6 py-4">{{ $item->notes ?? 'Tidak ada' }}</td>
                                <td class="px-6 py-4 text-right">
                                    <div class="flex justify-end space-x-3">
                                        @can('payment_expense.view')
                                            <a href="{{ route('payment_expense.show', $item->id) }}"
                                        ...
                                        @can('payment_expense.update')
                                            <a href="{{ route('payment_expense.edit', $item->id) }}"
                                        ...
                                        @can('payment_expense.delete')
                                            <form id="delete-form-{{ $item->id }}"
                                                action="{{ route('payment_expense.destroy', $item->id) }}" method="POST"
                                                style="display: none;">
                                                @csrf
                                                @method('DELETE')
                                            </form>
                            ...
                            <tr>
                                <td colspan="15" class="px-6 py-12 text-center">
                                    <div class="flex flex-col items-center text-gray-400">
                                        ...
                                        <p class="text-lg font-medium">Belum ada Payment Expenses</p>
                                        <a href="{{ route('payment_expense.create') }}"
