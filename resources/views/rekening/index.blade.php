                ...
                <div class="sticky top-0 z-20 px-6 py-5 border-b border-gray-100 flex justify-between items-center"
                    ...
                    @can('rekening.create')
                        <a href="{{ route('rekening.create') }}"
                            ...
                            <i class="fas fa-plus mr-2"></i> Add Rekening
                        </a>
                    @endcan
                </div>
            </div>
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @forelse ($data as $item)
                            <tr class="hover:bg-gray-50 transition-colors duration-150">
                                ...
                                <td class="px-4 py-2">{{ $item->atas_nama }}</td>
                                <td class="px-4 py-2">{{ $item->nama_bank ?? '-' }}</td>
                                <td class="px-4 py-2">{{ $item->no_rek ?? '-' }}</td>
                                <td class="px-4 py-2 text-right">
                                    <div class="flex justify-end space-x-3">
                                        @can('rekening.view')
                                            <a href="{{ route('rekening.show', $item->id) }}"
                            ...
                            <tr>
                                <td colspan="5" class="px-6 py-12 text-center">
                                    <div class="flex flex-col items-center text-gray-400">
                                        ...
                                        <p class="text-lg font-medium">Belum ada Rekening</p>
                                        <a href="{{ route('rekening.create') }}"
