    ...
    <div class="py-10">
        <div class="max-w-full mx-auto px-4 sm:px-6 lg:px-8">
                    ...
                    <div class="flex flex-wrap gap-2">
                        ...
                        @can('pembayaran_gaji.create')
                            <a href="{{ route('pembayaran_gaji.create') }}"
                                ...
                                <i class="fas fa-plus mr-2"></i> Add Salary Calculations
                            </a>
                        @endcan
            ...
            <div id="filterPanel"
                ...
                <form method="GET" action="{{ route('pembayaran_gaji.index') }}">
                    ...
                    <thead class="bg-gray-50 sticky top-0 z-10">
                        <tr>
                            ...
                            <th class="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">
                                Nama
                                Karyawan</th>
                            <th class="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">Nik
                            </th>
                            <th class="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">
                                Unit
                                Karyawan</th>
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @forelse ($data as $item)
                            <tr class="hover:bg-gray-50 transition-colors duration-150">
                                ...
                                <td class="px-2 py-1 text-center">{{ $item->employee->nama_karyawan }}</td>
                                <td class="px-2 py-1 text-center">{{ $item->employee->nik }}</td>
                                <td class="px-2 py-1 text-center">{{ $item->employee->unitKerja->nama_unit }}</td>
                                <td class="px-2 py-1 text-center">{{ $item->employee->levelKaryawan->nama_level }}</td>
                                <td class="px-2 py-1 text-center">{{ $item->periode_awal }}</td>
                                <td class="px-2 py-1 text-center">{{ $item->periode_akhir }}</td>
                                <td class="px-2 py-1 text-center">{{ $item->tanggal_pembayaran }}</td>
                                <td class="px-2 py-1 text-right">
                                    <div class="flex justify-end space-x-3">
                                        @can('pembayaran_gaji.view')
                                            <a href="{{ route('pembayaran_gaji.show', $item->id) }}"
                                        ...
                                        @can('pembayaran_gaji.update')
                                            <a href="{{ route('pembayaran_gaji.edit', $item->id) }}"
                                        ...
                                        @can('pembayaran_gaji.delete')
                                            <form id="delete-form-{{ $item->id }}"
                                                action="{{ route('pembayaran_gaji.destroy', $item->id) }}" method="POST"
                                                style="display: none;">
                                                @csrf
                                                @method('DELETE')
                                            </form>
                            ...
                            <tr>
                                <td colspan="10" class="px-6 py-12 text-center">
                                    <div class="flex flex-col items-center text-gray-400">
                                        ...
                                        <p class="text-lg font-medium">Belum ada Pembayaran Gaji</p>
                                        <a href="{{ route('pembayaran_gaji.create') }}"
