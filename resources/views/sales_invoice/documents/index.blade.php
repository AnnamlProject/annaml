    ...
    <div class="py-6">
        <div class="w-full px-4 sm:px-6 lg:px-8">
            <div class="bg-white shadow-md rounded-lg p-6">
                ...
                <h2 class="text-xl font-bold mb-4">Daftar Dokumen Sales Invoice</h2>
                        ...
                        <tbody>
                            @forelse ($documents as $index => $doc)
                                <tr>
                                    ...
                                    <td class="border px-3 py-2">{{ $doc->salesInvoice->invoice_number ?? '-' }}</td>
                                    <td class="border px-3 py-2">{{ $doc->document_name }}</td>
                                    <td class="border px-3 py-2">{{ $doc->created_at->format('d-m-Y H:i') }}</td>
                                    <td class="border px-3 py-2 text-center space-x-2">
                                        <a href="{{ route('sales_invoice.documents.download', [$doc->salesInvoice->id, $doc->id]) }}"
                                            class="text-blue-600 hover:underline">
                                            Download
                                        </a>
                                        <form
                                            action="{{ route('sales_invoice.documents.destroy', [$doc->salesInvoice->id, $doc->id]) }}"
