        ...
        <div class="max-w-full mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="bg-white rounded-2xl shadow-lg overflow-hidden border border-gray-200">
                ...
                <div
                    ...
                    <a href="{{ route('sales_deposits.create') }}"
                        ...
                        <i class="fas fa-plus mr-2"></i> Add Sales Deposits
                    </a>
                </div>
            </div>
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @forelse ($data as $item)
                            <tr class="hover:bg-gray-50 transition-colors duration-150">
                                ...
                                <td class="px-6 py-4">{{ $item->deposit_no }}</td>
                                <td class="px-6 py-4">{{ $item->jenis_pembayaran->nama_jenis }}</td>
                                <td class="px-6 py-4">{{ $item->account->nama_akun }}</td>
                                <td class="px-6 py-4">{{ $item->deposit_date }}</td>
                                <td class="px-6 py-4">{{ $item->customer->nama_customers ?? '-' }}</td>
                                <td class="px-6 py-4">{{ $item->deposit_reference }}</td>
                                <td class="px-6 py-4">{{ number_format($item->deposit_amount) }}</td>
                                <td class="px-6 py-4">{{ $item->comment }}</td>
                                <td class="px-6 py-4 text-right">
                                    <div class="flex justify-end space-x-3">
                                        <a href="{{ route('sales_deposits.show', $item->id) }}"
                                            class="text-blue-500 hover:text-blue-700 p-2 rounded-full hover:bg-blue-50 transition-colors"
                                            title="View">
                                            <i class="fas fa-eye"></i>
                                        </a>
                                        <a href="{{ route('sales_deposits.edit', $item->id) }}"
                                            class="text-yellow-500 hover:text-yellow-700 p-2 rounded-full hover:bg-yellow-50 transition-colors"
                                            title="Edit">
                                            <i class="fas fa-edit"></i>
                                        </a>
                                        <form action="{{ route('sales_deposits.destroy', $item->id) }}" method="POST"
                            ...
                            <tr>
                                <td colspan="15" class="px-6 py-12 text-center">
                                    <div class="flex flex-col items-center text-gray-400">
                                        ...
                                        <p class="text-lg font-medium">Belum ada Sales deposits</p>
                                        <a href="{{ route('sales_deposits.create') }}"
