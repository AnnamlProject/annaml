...
@section('content')
            ...
            <div class="bg-white shadow-lg rounded-xl p-6 border-t-4" style="border-color:{{ $themeColor }}">
                <form method="POST"
                    ...
                    <div style="border-radius: 8px; overflow: hidden; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
                        <table style="width: 100%; border-collapse: collapse;" id="tabel-jenis_hari">
                            <thead>
                            ...
                            <tbody id="tbody-jenis_hari">
                                <tr>
                                    ...
                                    <td style="padding: 12px; border: 1px solid #ddd;">
                                        <input type="text" name="nama[]" placeholder="Masukkan nama seperti Weekend"
                                            class="w-full border border-gray-300 rounded-lg px-4 py-2 bg-gray-50 focus:outline-none focus:ring-2 focus:ring-blue-500">
                                    </td>
                                    <td style="padding: 12px; border: 1px solid #ddd;">
                                        <input type="time" name="jam_mulai[]"
                                            value="{{ optional($jenis_hari)->jam_mulai ? \Carbon\Carbon::parse(optional($jenis_hari)->jam_mulai)->format('H:i') : '' }}"
                                            class="w-full border border-gray-300 rounded-lg px-4 py-2 bg-gray-50 focus:outline-none focus:ring-2 focus:ring-blue-500">
                                    </td>
                                    <td style="padding: 12px; border: 1px solid #ddd;">
                    ...
                    <div class="mt-6 justify-end flex space-x-4">
                        ...
                        <button type="submit"
                            ...
                            {{ isset($jenis_hari) ? 'Update' : 'Process' }}
                        </button>
                    </div>
                </form>
            </div>
    ...
    <script>
        ...
        const defaultJamMulai =
            "{{ isset($jenis_hari) && $jenis_hari->jam_mulai ? \Carbon\Carbon::parse($jenis_hari->jam_mulai)->format('H:i') : '' }}";
        const defaultJamSelesai =
            "{{ isset($jenis_hari) && $jenis_hari->jam_selesai ? \Carbon\Carbon::parse($jenis_hari->jam_selesai)->format('H:i') : '' }}";

        function tambahBaris() {
            let tbody = document.getElementById('tbody-jenis_hari');
            ...
            <td style="padding: 12px; border: 1px solid #ddd;">
                <select name="unit_kerja_id[]" class="w-full border border-gray-300 rounded-lg px-4 py-2 bg-gray-50 focus:outline-none focus:ring-2 focus:ring-blue-500">
                    <option value="">-- Pilih --</option>
                    @foreach ($unitKerja as $g)
                        <option value="{{ $g->id }}">{{ $g->nama_unit }}</option>
                    @endforeach
                </select>
            </td>
            ...
            <td style="padding: 12px; border: 1px solid #ddd;">
                <input type="time" name="jam_mulai[]" value="${defaultJamMulai}"
                    class="w-full border border-gray-300 rounded-lg px-4 py-2 bg-gray-50 focus:outline-none focus:ring-2 focus:ring-blue-500">
            </td>

            <td style="padding: 12px; border: 1px solid #ddd;">
                <input type="time" name="jam_selesai[]" value="${defaultJamSelesai}"
                    class="w-full border border-gray-300 rounded-lg px-4 py-2 bg-gray-50 focus:outline-none focus:ring-2 focus:ring-blue-500">
            </td>
