                    ...
                    <div class="flex flex-wrap gap-2">
                        ...
                        @can('create_budget.create')
                            <a href="{{ route('pengajuan.create') }}"
                                ...
                                <i class="fas fa-plus mr-2"></i> Add Pengajuan
                            </a>
                        @endcan
                    </div>
                </div>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @forelse ($pengajuan as $item)
                                <tr class="hover:bg-gray-50 transition-colors duration-150">
                                    ...
                                    <td class="px-2 py-1 text-center">{{ $item->no_pengajuan }}</td>
                                    <td class="px-2 py-1 text-center">{{ $item->tgl_pengajuan }}</td>
                                    <td class="px-2 py-1 text-center">{{ $item->rekening->no_rek ?? '-' }}</td>
                                    <td class="px-2 py-1 text-center">{{ $item->tgl_proses ?? '-' }}</td>
                                    <td class="px-2 py-1 text-center">{{ $item->status ?? 'Tidak Ada' }}</td>
                                    <td class="px-2 py-1 text-center">{{ $item->keterangan ?? 'Tidak Ada' }}</td>
                                    <td class="px-2 py-1 text-right">
                                        <div class="flex justify-end space-x-3">
                                            ...
                                            @can('create_budget.delete')
                                                <form id="delete-form-{{ $item->id }}"
                                                    action="{{ route('pengajuan.destroy', $item->id) }}" method="POST"
                                                    style="display: none;">
                                                    @csrf
                                                    @method('DELETE')
                                                </form>
