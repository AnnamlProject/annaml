    ...
    <div class="max-w-full mx-auto bg-white shadow-md rounded-xl p-8 mt-6">
        <form method="POST"
            ...
            <div class="grid grid-cols-4 gap-4 text-sm">
                ...
                <div>
                    <label for="level_karyawan_id" class="block text-sm font-medium text-gray-700 mb-1">
                        Level Karyawan
                    </label>
                    <select name="level_karyawan_id" id="level_karyawan_id"
                        ...
                        @foreach ($levelKaryawan as $g)
                            <option value="{{ $g->id }}"
                                {{ isset($target_unit) && $target_unit->level_karyawan_id == $g->id ? 'selected' : '' }}>
                                {{ $g->nama_level }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    <label for="komponen_penghasilan_id" class="block text-sm font-medium text-gray-700 mb-1">
                ...
                <div>
                    ...
                    <input type="text" name="bulan" value="{{ $target_unit->bulan }}"
                        class="w-full border border-gray-300 rounded-lg px-4 py-2 bg-gray-50 focus:outline-none focus:ring-2 focus:ring-blue-500"
                        required>
                </div>
                <div>
                    ...
                    <input type="text" name="tahun" value="{{ $target_unit->tahun }}"
                        class="w-full border border-gray-300 rounded-lg px-4 py-2 bg-gray-50 focus:outline-none focus:ring-2 focus:ring-blue-500"
                        required>
                </div>
                <div>
            ...
            <div class="flex justify-end">
                ...
                <button type="submit"
                    ...
                    {{ isset($target_unit) ? ' Process' : ' Process' }}
                </button>
            </div>
        </form>
    </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function loadKomponen(levelId, selectedId = null) {
            if (!levelId) {
                komponenSelect.innerHTML = '<option value="">-- Pilih Level Karyawan Dulu --</option>';
                return;
            }
            ...
            fetch('/komponen-by-level/' + levelId)
                ...
                .then(data => {
                    let options = '<option value="">-- Pilih Komponen Penghasilan --</option>';
                    if (data.length === 0) {
                        options = '<option value="">Tidak ada komponen untuk level ini</option>';
                    } else {
                        data.forEach(function(item) {
                            options +=
                                `<option value="${item.id}" ${selectedId == item.id ? 'selected' : ''}>${item.nama_komponen}</option>`;
                        });
                    }
        ...
        if (levelSelect) {
            ...
            @if (isset($target_unit) && $target_unit->level_karyawan_id)
                loadKomponen("{{ $target_unit->level_karyawan_id }}",
                    "{{ $target_unit->komponen_penghasilan_id }}");
            @endif
        }
    });
</script>
