                ...
                <div
                    ...
                    @can('receipts.access')
                        <a href="{{ route('receipts.create') }}"
                            ...
                            <i class="fas fa-plus mr-2"></i> Add Receipts
                        </a>
                    @endcan
                </div>
            </div>
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @forelse ($data as $item)
                            <tr class="hover:bg-gray-50 transition-colors duration-150">
                                ...
                                <td class="px-6 py-4">{{ $item->receipt_number }}</td>
                                <td class="px-6 py-4">{{ $item->customer->nama_customers ?? '-' }}</td>
                                <td class="px-6 py-4">{{ $item->depositTo->nama_account ?? '-' }}</td>
                                <td class="px-6 py-4">{{ $item->date }}</td>
                                <td class="px-6 py-4">{{ $item->comment }}</td>
                                <td class="px-6 py-4 text-right">
                                    <div class="flex justify-end space-x-3">
                                        @can('receipts.view')
                                            <a href="{{ route('receipts.show', $item->id) }}"
                                        ...
                                        @can('receipts.update')
                                            <a href="{{ route('receipts.edit', $item->id) }}"
                                        ...
                                        @can('receipts.delete')
                                            <form id="delete-form-{{ $item->id }}"
                                                action="{{ route('receipts.destroy', $item->id) }}" method="POST"
                                                style="display: none;">
                                                @csrf
                                                @method('DELETE')
                                            </form>
                            ...
                            <tr>
                                <td colspan="7" class="px-6 py-12 text-center">
                                    <div class="flex flex-col items-center text-gray-400">
                                        ...
                                        <p class="text-lg font-medium">Belum ada Receipts</p>
                                        <a href="{{ route('receipts.create') }}"
