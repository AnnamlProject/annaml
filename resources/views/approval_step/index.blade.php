            ...
            <div class="bg-white rounded-2xl shadow-lg overflow-hidden border border-gray-200">
                ...
                <div class="sticky top-0 z-20 px-6 py-5 border-b border-gray-100 flex justify-between items-center"
                    ...
                    <div class="flex flex-wrap gap-2">
                        ...
                        @can('approval_step.create')
                            <a href="{{ route('approval_step.create') }}"
                                ...
                                <i class="fas fa-plus mr-2"></i> Add Approval Step
                            </a>
                        @endcan
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @forelse ($data as $item)
                            <tr class="hover:bg-gray-50 transition-colors duration-150">
                                ...
                                <td class="px-4 py-2">{{ $item->jabatan->nama_jabatan ?? '-' }}</td>
                                <td class="px-4 py-2">{{ $item->step_order ?? '-' }}</td>
                                <td class="px-4 py-2 text-right">
                                    <div class="flex justify-end space-x-3">
                                        {{-- @can('wahana.view')
                                            <a href="{{ route('approval_step.show', $item->id) }}"
                                        ...
                                        @can('approval_step.update')
                                            <a href="{{ route('approval_step.edit', $item->id) }}"
                                        ...
                                        @can('approval_step.delete')
                                            <form id="delete-form-{{ $item->id }}"
                                                action="{{ route('approval_step.destroy', $item->id) }}" method="POST"
                                                style="display: none;">
                                                @csrf
                                                @method('DELETE')
                                            </form>
                            ...
                            <tr>
                                <td colspan="7" class="px-6 py-12 text-center">
                                    <div class="flex flex-col items-center text-gray-400">
                                        ...
                                        <p class="text-lg font-medium">Belum ada Approval Step</p>
                                        <a href="{{ route('approval_step.create') }}"
