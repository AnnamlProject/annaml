        ...
        <div class="max-w-full mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="bg-white rounded-2xl shadow-lg overflow-hidden border border-gray-200">
                ...
                <div
                    ...
                    <a href="{{ route('linkedAccountSales.create') }}"
            ...
            <div class="relative overflow-x-auto" style="max-height: calc(100vh - 250px); overflow-y: auto;">
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @forelse ($linkedAccountSales as $item)
                            <tr class="hover:bg-gray-50 transition-colors duration-150">
                                <td class="px-6 py-4 text-sm text-gray-500">{{ $loop->iteration }}</td>
                                <td class="px-6 py-4">{{ $item->kode }}</td>
                                <td class="px-6 py-4">{{ $item->akun->kode_akun . '-' . $item->akun->nama_akun ?? '-' }}</td>
                                <td class="px-6 py-4 text-right">
                                    <div class="flex justify-end space-x-3">
                                        <form action="{{ route('linkedAccountSales.destroy', $item->id) }}" method="POST"
