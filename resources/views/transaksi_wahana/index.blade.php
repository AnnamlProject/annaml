    ...
    <div class="py-10">
        <div class="max-w-full mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="bg-white rounded-2xl shadow-lg overflow-hidden border border-gray-200">
                ...
                <div class="sticky top-0 z-20 px-6 py-5 border-b border-gray-100 flex justify-between items-center"
                    ...
                    <div class="flex flex-wrap gap-2">
                        ...
                        @can('transaksi_wahana.create')
                            <a href="{{ route('transaksi_wahana.create') }}"
                                ...
                                <i class="fas fa-plus mr-2"></i> Add Transaksi
                            </a>
                        @endcan
            ...
            <div id="filterPanel"
                ...
                <form method="GET" action="{{ route('transaksi_wahana.index') }}">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @forelse ($data as $item)
                            <tr class="hover:bg-gray-50 transition-colors duration-150">
                                ...
                                <td class="px-4 py-2 text-center">{{ $item->jumlah_pengunjung ?? '-' }}</td>
                                <td class="px-4 py-2 text-right">
                                    <div class="flex justify-end space-x-3">
                                        @can('transaksi_wahana.view')
                                            <a href="{{ route('transaksi_wahana.show', $item->id) }}"
                                        ...
                                        @can('transaksi_wahana.update')
                                            <a href="{{ route('transaksi_wahana.edit', $item->id) }}"
                                        ...
                                        @can('transaksi_wahana.delete')
                                            <form id="delete-form-{{ $item->id }}"
                                                action="{{ route('transaksi_wahana.destroy', $item->id) }}" method="POST"
                                                style="display: none;">
                                                @csrf
                                                @method('DELETE')
                                            </form>
            ...
            <div id="fileModal"
                ...
                <div class="bg-white rounded-xl shadow-lg p-6 w-full max-w-md">
                    ...
                    <div class="space-y-3 text-sm text-gray-700">
                        <a href="{{ asset('template/template_import_transaksi_wahana.xlsx') }}" download
                            class="block hover:bg-gray-50 p-2 rounded-lg">
                            <i class="fas fa-file-excel mr-2 text-green-600"></i> Download Template Excel
                        </a>
                        <a href="{{ route('export.TransaksiWahana') }}" class="block hover:bg-gray-50 p-2 rounded-lg">
                            <i class="fas fa-file-download mr-2 text-blue-500"></i> Export
                        </a>
                        <form action="{{ route('import.TransaksiWahana') }}" method="POST"
