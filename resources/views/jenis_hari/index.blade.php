        ...
        <div class="max-w-full mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="bg-white rounded-2xl shadow-lg overflow-hidden border border-gray-200">
                ...
                <div class="sticky top-0 z-20 px-6 py-5 border-b border-gray-100 flex justify-between items-center"
                    ...
                    @can('jenis_hari.create')
                        <a href="{{ route('jenis_hari.create') }}"
                            ...
                            <i class="fas fa-plus mr-2"></i> Add Jenis Hari
                        </a>
                    @endcan
                    ...
                    <thead class="bg-gray-50 sticky top-0 z-10">
                        <tr>
                            ...
                            <th class="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">
                                Jumlah Pengunjung Min</th>
                            <th class="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">
                                Jumlah Pengunjung Max</th>
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                Deksripsi</th>
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @forelse ($data as $item)
                            <tr class="hover:bg-gray-50 transition-colors duration-150">
                                ...
                                <td class="px-4 py-2 text-center">{{ number_format($item->jumlah_pengunjung_min) }}</td>
                                <td class="px-4 py-2 text-center">{{ number_format($item->jumlah_pengunjung_max) }}</td>
                                <td class="px-4 py-2">{{ $item->deskripsi ?? '-' }}</td>
                                <td class="px-4 py-2 text-right">
                                    <div class="flex justify-end space-x-3">
                                        @can('jenis_hari.view')
                                            <a href="{{ route('jenis_hari.show', $item->id) }}"
                                        ...
                                        @can('jenis_hari.update')
                                            <a href="{{ route('jenis_hari.edit', $item->id) }}"
                                        ...
                                        @can('jenis_hari.delete')
                                            <form id="delete-form-{{ $item->id }}"
                                                action="{{ route('jenis_hari.destroy', $item->id) }}" method="POST"
                                                style="display: none;">
                                                @csrf
                                                @method('DELETE')
                                            </form>
                            ...
                            <tr>
                                <td colspan="7" class="px-6 py-12 text-center">
                                    <div class="flex flex-col items-center text-gray-400">
                                        ...
                                        <p class="text-lg font-medium">Belum ada jenis hari</p>
                                        <a href="{{ route('jenis_hari.create') }}"
