...
@section('content')
    <div class="py-10 bg-gradient-to-br from-gray-50 to-blue-50 min-h-screen">
        ...
        <div class="max-w-full mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white shadow-lg rounded-xl p-6 border-t-4" style="border-color:{{ $themeColor }}">
                <form method="POST"
                    action="{{ isset($fiscal_account) ? route('fiscal_account.update', $fiscal_account->id) : route('fiscal_account.store') }}">
                    @csrf
                    @if (isset($fiscal_account))
                        @method('PUT')
                    @endif
                    ...
                    <h4 class="font-semibold text-lg text-gray-800 mt-8 mb-4 border-l-4 border-blue-500 pl-2">
                        Fiscal Account Create
                    </h4>

                    <div style="border-radius: 8px; overflow: hidden; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
                        <table style="width: 100%; border-collapse: collapse;" id="tabel-fiscal_account">
                            <thead>
                                <tr style="background-color: #f5f5f5; font-weight: bold;">
                                    <th style="padding: 12px; border: 1px solid #ddd; width: 50px;">No.</th>
                                    <th style="padding: 12px; border: 1px solid #ddd;">Kode Akun</th>
                                    <th style="padding: 12px; border: 1px solid #ddd;">Nama Akun</th>
                                    <th style="padding: 12px; border: 1px solid #ddd; width: 70px;">Aksi</th>
                                </tr>
                            </thead>
                            <tbody id="tbody-fiscal_account">
                                <tr>
                                    <td style="padding: 12px; border: 1px solid #ddd;">1</td>
                                    <td style="padding: 12px; border: 1px solid #ddd;">
                                        <input type="text" name="kode_akun[]" placeholder="Masukkan kode akun fiscal"
                                            class="w-full border border-gray-300 rounded-lg px-4 py-2 bg-gray-50 focus:outline-none focus:ring-2 focus:ring-blue-500">
                                    </td>
                                    <td style="padding: 12px; border: 1px solid #ddd;">
                                        <input type="text" name="nama_akun[]"
                                            class="w-full border border-gray-300 rounded-lg px-4 py-2 bg-gray-50 focus:outline-none focus:ring-2 focus:ring-blue-500"
                                            placeholder="Masukkan nama akun fiscal">
                                    </td>
                    ...
                    <div class="mt-6 justify-end flex space-x-4">
                        ...
                        <button type="submit"
                            ...
                            {{ isset($fiscal_account) ? 'Update' : 'Process' }}
                        </button>
                    </div>
                </form>
            </div>
    ...
    <script>
        function tambahBaris() {
            let tbody = document.getElementById('tbody-fiscal_account');
            ...
            <td style="padding: 12px; border: 1px solid #ddd;">
                <input type="text" name="nama_akun[]" placeholder="Masukkan nama akun fiscal"
                    class="w-full border border-gray-300 rounded-lg px-4 py-2 bg-gray-50 focus:outline-none focus:ring-2 focus:ring-blue-500">
            </td>
