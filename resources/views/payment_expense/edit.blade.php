...
@section('content')
    ...
    <div class="py-4">
        <div class="w-full px-2">
            ...
            <div class="bg-white shadow rounded p-4">
                ...
                <h2 class="text-lg font-bold mb-4">Edit Payment Expense</h2>

                <form method="POST" action="{{ route('payment_expense.update', $payment_expense->id) }}">
                    @csrf
                    @method('PUT')
                        ...
                        <div class="grid grid-cols-3 gap-4 text-sm">
                            <div>
                                ...
                                <input type="date" name="date" class="w-full border rounded px-2 py-1 text-sm"
                                    value="{{ old('date', $payment_expense->date) }}" required>
                            </div>
                            <div>
                                ...
                                <select name="account_header_id" id="account_header_id"
                                    ...
                                    @foreach ($account as $acc)
                                        <option value="{{ $acc->id }}"
                                            {{ old('from_account_id', $payment_expense->from_account_id) == $acc->id ? 'selected' : '' }}>
                            ...
                            <div>
                                ...
                                <input type="text" name="source" placeholder="Masukkan Source"
                                    ...
                                    value="{{ old('source', $payment_expense->source) }}" required>
                            </div>
                        </div>
                                    ...
                                    <tr class="item-row" data-index="{{ $i }}">
                                        ...
                                        <td>
                                            <input type="text" name="items[{{ $i }}][deskripsi]"
                                                class="w-full border rounded text-sm" value="{{ $detail->deskripsi }}">
                                        </td>
                                        <td>
                                            <input type="number" step="0.01" name="items[{{ $i }}][amount]"
                                                class="amount-input w-full border rounded text-sm"
                                                value="{{ $detail->amount }}">
                                        </td>
                                        <td>
    ...
    <script>
        ...
        let rowIndex = {{ count($payment_expense->details) }};

        function generateRow(index) {
            return `
        <tr class="item-row" data-index="${index}">
            <td>
                <select name="items[${index}][account_id]" class="account-select w-full border rounded text-sm" required>
                    <option value="">-- Account --</option>
                    @foreach ($account_beban as $acc)
            ...
            <td>
                <input type="text" name="items[${index}][deskripsi]" class="w-full border rounded text-sm" />
            </td>
            <td>
                <input type="number" step="0.01" name="items[${index}][amount]" class="amount-input w-full border rounded text-sm" />
            </td>
            <td>
            ...
            <td class="text-center">
                <button type="button" class="remove-row bg-red-500 text-white rounded text-xs px-2">Hapus</button>
            </td>
        </tr>`;
        }
        ...
        $(document).ready(function() {
            ...
            // panggil recalcTotals pertama kali
            recalcTotals();
            ...
            // hitung ulang setiap input berubah
